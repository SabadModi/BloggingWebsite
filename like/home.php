		...
		while($row=mysqli_fetch_array($query)){
			?>
				...
				<div>
						...
						<?php
							$query1=mysqli_query($conn,"select * from `like` where postid='".$row['id']."' and userid='".$_SESSION['userid']."'");
							if (mysqli_num_rows($query1)>0){
								?>
                                <button value="<?php echo $row['id']; ?>" class="unlike">Unlike</button>
								<?php
							}
							else{
								?>
								<button value="<?php echo $row['id']; ?>" class="like">Like</button>
								<?php
							}
						?>
					<span id="show_like<?php echo $row['id']; ?>">
						<?php
							$query3=mysqli_query($conn,"select * from `like` where postid='".$row['id']."'");
